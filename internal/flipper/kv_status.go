...
var (
	statusKVName  = string(types.ConditionKindCookieFlip)
	defaultKVOpts = []kv.Option{
		kv.WithDescription("cookieflipper condition status tracking"),
...
func (s *statusKVPublisher) Publish(ctx context.Context, cookieID, conditionID string, lastRevision uint64, payload []byte) (revision uint64) {
	...
	if err != nil {
		...
		span.AddEvent("status publish failure",
			trace.WithAttributes(
				...
				attribute.String("cookieID", cookieID),
...
//		FlipperID: hCtx.FlipperID.String(),
