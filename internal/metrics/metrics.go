...
func init() {
	...
	ConditionRunTimeSummary = promauto.NewSummaryVec(
		prometheus.SummaryOpts{
			Name: "cookieflipper_condition_duration_seconds",
	...
	StoreQueryErrorCount = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cookieflipper_store_query_error_count",
	...
	NATSErrors = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cookieflipper_nats_errors",
...
func RegisterSpanEvent(span trace.Span, condition *cptypes.Condition, workerID, cookieID, event string) {
	span.AddEvent(event, trace.WithAttributes(
		attribute.String("workerID", workerID),
		attribute.String("serverID", cookieID),
